...
namespace gl
{
    ...
    interop<T>::interop(unsigned int * BO, int size)
    {
        ...
        gpuErrchk(cudaMalloc(&data_device, sizeof(T)*size));
        gpuErrchk(cudaGraphicsGLRegisterBuffer(&gl_buffer, *BO,
                                 cudaGraphicsMapFlagsNone));
        gpuErrchk(cudaGraphicsMapResources(1, &gl_buffer, 0));
        gpuErrchk(cudaGraphicsResourceGetMappedPointer((void**)&gl_data,
                   nullptr, gl_buffer));
        return;
    }
