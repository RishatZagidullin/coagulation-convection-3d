...
{
	checkCudaErrors(cudaMalloc(&d_U, this->get_rows_number() * this->rank*sizeof(double)));
	checkCudaErrors(cudaMalloc(&d_V, this->get_columns_number() * this->rank*sizeof(double)));
...
void TCross_Parallel_v1::smol_conv_discrete(double * &x, cufftHandle & plan, cublasHandle_t &handle, double * &result)
{
	int N = this->rows_number;
	...
	this->smol_conv(x, plan, handle, result);
        gpuErrchk(cudaMemset(&result[N-1], 0.0, sizeof(double)));
	//smol_conv_bubble<<<grid, block>>>(result, N);
}
...
void TCross_Parallel_v1::smol_conv(double * &x, cufftHandle & plan, cublasHandle_t &handle, double * &result)
{
	...
	double *d_ones;
	cudaMalloc(&d_ones, R * sizeof(double));
	cudaMemcpy(d_ones, ones, R*sizeof(double), 
	           cudaMemcpyHostToDevice);

       	double *temp_res;
	cudaMalloc(&temp_res, R*N*sizeof(double));

	int mem_size = sizeof(cufftDoubleComplex) * N * R;

	cufftDoubleComplex *d_ub;
	cudaMalloc(&d_ub, mem_size);

	cufftDoubleComplex *d_vb;
	cudaMalloc(&d_vb, mem_size);

	for (int r = 0; r < R; r++)
      	{
		MulAndComplexCast<<<grid, block >>>(&d_ub[r*M], 
		                                    &d_U[r*M], x, N);
		MulAndComplexCast<<<grid, block >>>(&d_vb[r*M], 
		                                    &d_V[r*N], x, M);
	}
	...
	ComplexPointwiseMulAndScale<<<grid, block >>>(d_ub, d_vb,
	                                              N * R, 1.0l / N);
	...
	for (int i = 0; i < R; i++)
	{
		DoubleCastFromComplex<<<grid, block>>>(&temp_res[i*N],
		                                       &d_ub[i*N], N);
	}
	cublasDgemv(handle, CUBLAS_OP_N, N, R, &alpha, temp_res, N, 
	            d_ones, 1, &beta, result, 1);
...
void TCross_Parallel_v1::matvec(double* &x, cublasHandle_t &handle, double* &result, const char &option)
{
	...
	if (option == 'f')
	{
		...
		double *d_buff;
		cudaMalloc(&d_buff, R*sizeof(double));
