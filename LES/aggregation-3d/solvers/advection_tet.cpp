...
void SpaceHeter3d_tet<F,I>::initialize_again(std::vector<F> &val, Vector3d<F> const  &source)
{
    #pragma omp parallel for
    for (int i = 0; i < size; i++)
    {
        ...
        F z_sq = (data.cell_centroids[i].z - source.z) * 
            ...
            val[i] += 0.3*exp(-10.0*(z_sq+y_sq+x_sq));
    }
}
...
{
    //initialize(f, Vector3d<F>{0.0,-3.0,0.0});
    for (int j = 0; j < 3; j++)
...
bool is_vert_boundary(double x, double y, double z){
    if (fabs(x+2) < 1e-2 || fabs(y+3) < 1e-2 || fabs(z+2) < 1e-2)
        return true;
    if (fabs(x-2) < 1e-2 || fabs(y-3) < 1e-2 || fabs(z-10) < 1e-2)
        return true;
    return false;
}
...
template<typename F, typename I>
void SpaceHeter3d_tet<F,I>::find_inds(Vector3d<F> const * cc, int n_vort)
{
    std::cout << "searching for inds...\n";
    inds.resize(size);
    weights.resize(size);
    #pragma omp parallel for
    for (int i = 0; i < size; i++)
    {
        F tot = 0;
        for (int j = 0; j < n_vort; j++)
        {
            Vector3d<F> diff = Vector3d<F>(data.cell_centroids[i].x-cc[j].y,
                                           data.cell_centroids[i].y-cc[j].z, 
                                           data.cell_centroids[i].z-cc[j].x);
            //std::cout << cc[j].y << " " << cc[j].z << " " << cc[j].x << "\n";
            //std::cout << data.cell_centroids[i].x << " " << data.cell_centroids[i].y << " " << data.cell_centroids[i].z << "\n\n";
            F dist = pow(diff.x*diff.x+diff.y*diff.y+diff.z*diff.z, 0.5);
            if (dist < 0.2)
            {
                inds[i].push_back(j);
                weights[i].push_back(1./dist);
                tot += 1./dist;
            }
        }
        //std::cout << weights[i].size() << "\n\n\n";
...
template<typename F, typename I>
void SpaceHeter3d_tet<F,I>::find_vels(Vector3d<F> const * vels)
{
    for (int i = 0; i < size; i++)
    {
        u[0][i] = 0.0;
        ...
        {
            u[0][i] += vels[inds[i][j]].y*weights[i][j];
            u[1][i] += -vels[inds[i][j]].z*weights[i][j];
            u[2][i] += -vels[inds[i][j]].x*weights[i][j];
        }
        //u[0][i] = 0.0;
        //u[1][i] = -0.5;
        //u[2][i] = -1.0;
                ...
                std::vector<std::vector<std::vector<F>>> const &vel)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        f[i] -= dt*(temp + 1./3.*f[i] * sigma(data.cell_centroids[i].z+2.0, (F) 0.02, 50, 600) +
                           1./3.*f[i] * sigma(data.cell_centroids[i].y+3.0, (F) 0.05, 20, 120) +
                           1./3.*f[i] * sigma(data.cell_centroids[i].x+2.0, (F) 0.02, 50, 200)
                   )/
                   (1.+1./3.*dt*(sigma(data.cell_centroids[i].z+2.0, (F) 0.02, 50, 600) +
                                 sigma(data.cell_centroids[i].y+3.0, (F) 0.05, 20, 120) +
                                 sigma(data.cell_centroids[i].x+2.0, (F) 0.02, 50, 200)
                                 )
                   );
    }
}
...
double W(double v)
{
    return v < 0.5 ? 1. - pow(v/0.5, 0.4) : 0.0;
}
