...
public:
    ...
    std::vector<std::vector<I>> inds;
    std::vector<std::vector<F>> weights;

    void find_inds(Vector3d<F> const * cc, int n_vort);
    void find_vels(Vector3d<F> const * vels);
    ...
    void initialize_again(std::vector<F> &input, Vector3d<F> const & source);
