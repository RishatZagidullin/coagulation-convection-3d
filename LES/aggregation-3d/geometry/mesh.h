...
public:
    ...
    geometry3d(std::string smin, std::string smout)
    {
        ...
        //                    cell_size = 0.15);
        Mesh_criteria crits(edge_size = 0.3, facet_size = 0.3,
                            cell_size = 0.35);
                  ...
                  it!=tet.finite_cells_end(); it++)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                {
                    std::cout << "right now shouldn't happen\n";
                    bad_cell = true;
                    break;
                }
            }
...
public:
    ...
    geometry_vis(geometry3d const & g)
    {
                  ...
                  it!=g.tet.finite_vertices_end(); it++)
        {
            ...
            vertices[c*6] = it->point().z();
            vertices[c*6+1] = it->point().y();
            vertices[c*6+2] = it->point().x();
            vertices[c*6+3] = 0.0;
            vertices[c*6+4] = 0.0;
            vertices[c*6+5] = 0.0;
            c++;
                  ...
                  it != g.tet.finite_facets_end(); it++)
        {
            ...
            {
                //if (it->first->vertex(i)->point().x() > 0 &&
                //    it->first->vertex(i)->point().y() > 0 &&
                //    it->first->vertex(i)->point().z() > 0)
                //{
                //    bad_face = true;
                //    break;
                //}
    ...
    void upd_indices(geometry3d const & g, std::vector<double> &f, std::vector<int> * inc)
    {
        int c = 0;
        for (auto it(g.tet.finite_facets_begin());
                  it != g.tet.finite_facets_end(); it++)
        {
            ...
            for (int i = 0; i < 4; i++) if (i!=it->second)
            {
                ...
                int ind = it->first->vertex(i)->index();
                double val = 0.0;
                for (int j = 0; j < inc[ind].size(); j++)
                    val += f[inc[ind][j]];
                val /= ((float) inc[ind].size());
                if (val < 0.001)
                {
                    bad_face = true;
                    break;
